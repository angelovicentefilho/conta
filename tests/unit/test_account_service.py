...
class MockAccountRepository:
    ...
    async def get_by_name_and_user(self, name: str, user_id: UUID) -> Account:
        for account in self.accounts.values():
            if (account.name.lower() == name.lower() and
                    account.user_id == user_id and account.is_active):
                return account
        return None
...
class TestPrimaryAccount:
    ...
    async def test_set_primary_account_not_found(self, account_service, user_id):
