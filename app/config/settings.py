...
Settings, permitindo carregamento automático de variáveis de ambiente e
validação.
"""
...
class Settings(BaseSettings):
    ...
    reload: bool = Field(
        default=True, description="Auto-reload em desenvolvimento"
    )

    # Informações da aplicação
    app_name: str = Field(
        default="Financeiro Backend", description="Nome da aplicação"
    )
    app_version: str = Field(
        default="0.1.0", description="Versão da aplicação"
    )
    ...
    algorithm: str = Field(
        default="HS256", description="Algoritmo de criptografia JWT"
    )
    ...
    mongodb_database: str = Field(
        default="financeiro_db", description="Nome do banco de dados"
    )
    mongodb_min_connections: int = Field(
        default=10, description="Número mínimo de conexões no pool"
    )
    mongodb_max_connections: int = Field(
        default=100, description="Número máximo de conexões no pool"
    )
    ...
    redis_ttl: int = Field(
        default=3600, description="TTL padrão para cache em segundos"
    )
    ...
    smtp_password: Optional[str] = Field(
        default=None, description="Senha SMTP"
    )
    ...
    test_mongodb_database: str = Field(
        default="financeiro_test_db", description="Banco de dados para testes"
    )
    test_redis_url: str = Field(
        default="redis://localhost:6379/1", description="URL Redis para testes"
    )
